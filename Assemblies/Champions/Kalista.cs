...
namespace Assemblies.Champions
{
    internal class Kalista : Champion
    {
        ...
        //public string[] JungleMinions => JungleMinions1;
        //public string[] JungleMinions1 { get; } = { "SRU_Dragon", "Sru_Crab", "SRU_Baron", "SRU_Blue", "SRU_Red" };
        ...
        {
            var jungleMinion = MinionManager.GetMinions(
            ...
            {
                foreach (var jungMinion in jungleMinion)
                {
                    var buff =
        ...
        private void AutoKillMinion()
        {
            ...
            var count = minions.Count(minion => E.GetDamage(minion) > minion.Health);
        ...
        private void OnDraw(EventArgs args)
        {
            ...
            if (Menu.Item("drawStacks").GetValue<bool>())
            {
                ...
                var buffCount = 0;
                foreach (var buff in target.Buffs.Where(buff => buff.Name == "KalistaExpungeMarker"))
                {
            ...
            if (Menu.Item("drawFlee").GetValue<bool>())
            {
                foreach (var pos in jumpPos)
                {
                    ...
                    if (Player.Distance(pos.Key) <= 35f || Player.Distance(pos.Value) <= 35f)
                    {
        ...
        {
            var targets = MinionManager.GetMinions(
                Player.ServerPosition, Orbwalking.GetRealAutoAttackRange(Player), MinionTypes.All, MinionTeam.NotAlly);
            Obj_AI_Base bestTarget = null;

            foreach (var target in
                targets.Where(minion => minion.Distance(Player) <= Orbwalking.GetRealAutoAttackRange(Player)))
            {
                bestTarget = target;
            }
            ...
            if (Menu.Item("useQF").GetValue<bool>())
            {
                ...
                foreach (var xTo in from pos in jumpPos
                    where
                        ObjectManager.Player.Distance(pos.Key) <= 35f || ObjectManager.Player.Distance(pos.Value) <= 35f
                    let xTo = pos.Value
                    select
